...
const CopyButton: React.FC<{ textToCopy: string, absolute?: boolean }> = ({ textToCopy, absolute = true }) => {
    ...
    return (
        <button
            type="button"
            onClick={handleCopy}
            className={`${absolute ? 'absolute top-1/2 right-2 -translate-y-1/2' : ''} p-1.5 bg-slate-500/50 hover:bg-slate-400/70 rounded-md text-slate-300 hover:text-white transition-colors`}
...
const PromptItem: React.FC<PromptItemProps> = ({ prompt, index, onUpdate, onRemove, onEdit, movingPromptIndex, onSelectToMove, onMoveTo, onCancelMove }) => {
    ...
    const handleToggle = (isEnabled: boolean) => {
        onUpdate(index, { ...prompt, enabled: isEnabled });
    };

    return (
        <div 
            className={`bg-slate-800 rounded-lg border border-slate-700 transition-all duration-200 ${isBeingMoved ? 'ring-2 ring-sky-500 shadow-lg' : ''} ${isMoveMode && !isBeingMoved ? 'opacity-60' : ''}`}
        >
            <div className="flex items-center p-3">
                <div className="flex items-center gap-2 flex-shrink-0 mr-4" onClick={(e) => e.stopPropagation()}>
                    <ToggleInput label="" checked={prompt.enabled ?? false} onChange={handleToggle} />
                </div>
                <div className="flex-grow cursor-pointer overflow-hidden" onClick={(e) => { e.stopPropagation(); onEdit(index); }}>
                    <div className="flex flex-col sm:flex-row sm:items-baseline gap-1 sm:gap-3 truncate">
                        <h3 className={`font-medium text-base flex-shrink-0 ${prompt.enabled ? 'text-slate-200' : 'text-slate-500 line-through'}`}>
                     ...
                     {isMoveMode ? (
                        isBeingMoved ? (
                            <button
                                ...
                                aria-label={`Hủy di chuyển lời nhắc "${prompt.name}"`}
                            >
                            ...
                            <button
                                ...
                                aria-label={`Di chuyển lời nhắc đã chọn đến vị trí này, trước "${prompt.name}"`}
                            >
                                Di chuyển đến đây
                            </button>
                        )
                        ...
                        <button
                            ...
                            aria-label={`Chọn lời nhắc "${prompt.name}" để di chuyển`}
                        >
...
const FilterButton: React.FC<{
    filterId: PromptFilter;
    currentFilter: PromptFilter;
    onClick: (filterId: PromptFilter) => void;
    children: React.ReactNode;
}> = ({ filterId, currentFilter, onClick, children }) => (
  <button
    onClick={() => onClick(filterId)}
    className={`px-4 py-1.5 text-sm font-semibold rounded-md transition-colors duration-200 focus:outline-none focus-visible:ring-2 focus-visible:ring-sky-400 ${
            ...
            <div className="p-4 pt-0">
                <div className="relative">
                    <textarea
                        ...
                        value={finalPrompt}
                        rows={15}
                        className="w-full bg-slate-900/50 border border-slate-700 rounded-md p-3 text-slate-300 font-mono text-xs focus:outline-none"
                    />
                     <div className="absolute top-2 right-2">
                        <CopyButton textToCopy={finalPrompt} absolute={false} />
                    </div>
                </div>
            </div>
        </details>
...
export const PromptEditor: React.FC<PromptEditorProps> = ({ preset, onUpdate }) => {
    ...
    const handleSavePrompt = useCallback((updatedPrompt: PromptEntry) => {
        if (editingPromptIndex !== null) {
            handleUpdatePrompt(editingPromptIndex, updatedPrompt);
        }
    ...
    const filteredPrompts = useMemo(() => {
     ...
     const handleSelectToMove = useCallback((originalIndex: number) => {
        setMovingPromptIndex(originalIndex);
    }, []);
    ...
    const handleMoveTo = useCallback((destinationOriginalIndex: number) => {
        ...
        const effectiveDestinationIndex = destinationOriginalIndex > movingPromptIndex 
            ? destinationOriginalIndex - 1 
    ...
    const handleExportFilteredPrompts = useCallback(() => {
        ...
        let content: string;
        if (filter === 'all') {
            content = filteredPrompts
                .map(({ prompt }) => `--- Lời nhắc: ${prompt.name || 'Không có tiêu đề'} ---\n\n${prompt.content || ''}`)
                .join('\n\n\n');
        } else {
            content = filteredPrompts
        ...
        <div className="space-y-6">
            ...
            <div className="flex flex-col sm:flex-row justify-between items-center gap-4 p-2 bg-slate-800/50 rounded-lg">
                <div className="p-1 bg-slate-900 rounded-lg flex space-x-1">
                    <FilterButton filterId="all" currentFilter={filter} onClick={setFilter}>Tất cả</FilterButton>
                    <FilterButton filterId="enabled" currentFilter={filter} onClick={setFilter}>Đã bật</FilterButton>
                    <FilterButton filterId="disabled" currentFilter={filter} onClick={setFilter}>Đã tắt</FilterButton>
                </div>
                 <button
                    onClick={handleExportFilteredPrompts}
                    disabled={filteredPrompts.length === 0}
                    className="flex items-center gap-2 px-4 py-1.5 text-sm font-semibold rounded-md transition-colors duration-200 focus:outline-none focus-visible:ring-2 focus-visible:ring-sky-400 text-sky-300 bg-sky-800/50 hover:bg-sky-700/60 disabled:bg-slate-700 disabled:text-slate-500 disabled:cursor-not-allowed"
                >
            ...
            <div id="prompt-list" className="space-y-3">
                {filteredPrompts.map(({ prompt, originalIndex }) => (
                    <PromptItem
                        ...
                        onSelectToMove={handleSelectToMove}
                ...
                {filteredPrompts.length === 0 && (
                    <div className="text-center py-8 text-slate-500">
                        <p>Không có lời nhắc nào khớp với bộ lọc này.</p>
                    </div>
                )}
            </div>
            ...
            <button
                ...
                className="w-full bg-slate-700 hover:bg-slate-600 text-sky-400 font-semibold py-3 px-4 rounded-lg transition-colors duration-200 flex items-center justify-center gap-2 border border-slate-600"
            >
                <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" viewBox="0 0 20 20" fill="currentColor">
            ...
            <PromptEditModal
                isOpen={editingPromptIndex !== null}
                prompt={editingPromptIndex !== null ? prompts[editingPromptIndex] : null}
